...
<div class="flex items-center justify-between mb-8">
    <h1 class="text-3xl font-bold text-gray-800 flex items-center">
        <i class="fas fa-search text-guide-blue mr-3"></i>
        Available Trips
    </h1>
    <a href="{{ route('guide.trips') }}" class="flex items-center text-guide-blue hover:underline">
...
<div class="bg-white rounded-xl shadow-lg overflow-hidden">
    @if(isset($availableTrips) && count($availableTrips) > 0)
        <div class="bg-gray-50 p-4 border-b">
            <div class="flex items-center space-x-2">
                <i class="fas fa-info-circle text-guide-blue"></i>
                <p class="text-gray-700">Below are trips that need guiding services. Click on a trip to view details or join.</p>
            </div>
        </div>
                ...
                <div class="bg-white border rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-sm text-gray-700 mb-3">
                            <span class="font-medium">{{ $trip->travellers->count() }}</span> travelers · 
                            <span class="font-medium">{{ \Carbon\Carbon::parse($trip->start_date)->diffInDays($trip->end_date) + 1 }}</span> days
                        </p>
                        
                        <div class="flex space-x-2 mt-4">
                            <a href="{{ route('guide.trip.details', $trip->id) }}" class="flex-1 bg-guide-blue hover:bg-blue-700 text-white text-center py-2 px-4 rounded-lg transition-colors">
                                View Details
                            </a>
                            @if(!$isAssigned)
                                <form action="{{ route('guide.join-trip') }}" method="POST" class="flex-1">
                                    @csrf
                                    <input type="hidden" name="trip_id" value="{{ $trip->id }}">
                                    <button type="submit" class="w-full bg-guide-green hover:bg-green-600 text-white py-2 px-4 rounded-lg transition-colors">
                                        Join Trip
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('guide.withdraw-trip', $trip->id) }}" method="POST" class="flex-1">
                                    @csrf
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="text-gray-500">There are currently no trips available for guides.</p>
        </div>
    @endif
</div>
@endsection
